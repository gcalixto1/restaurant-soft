...
############################ FUNCION ACTUALIZAR PRODUCTOS ############################
    ...
    if (isset($_POST["codingrediente"])) {
		...
		$ingrediente = limpiar($_POST['codingrediente']);
		$repeated = array_filter(array_count_values($ingrediente), function($count) {
			return $count > 1;
		});
		foreach ($repeated as $key => $value) {
...
public function RegistrarArqueoCaja()
{
	...
	if($num == 0)
	{
		...
		if (limpiar(isset($_POST['ingresos']))) { $ingresos = limpiar($_POST['ingresos']); } else { $ingresos =''; }
		if (limpiar(isset($_POST['egresos']))) { $egresos = limpiar($_POST['egresos']); } else { $egresos =''; }
		if (limpiar(isset($_POST['creditos']))) { $creditos = limpiar($_POST['creditos']); } else { $creditos =''; }
		if (limpiar(isset($_POST['abonos']))) { $abonos = limpiar($_POST['abonos']); } else { $abonos =''; }
	if (limpiar(isset($_POST['dineroefectivo']))) { $dineroefectivo = limpiar($_POST['dineroefectivo']); } else { $dineroefectivo =''; }
		if (limpiar(isset($_POST['diferencia']))) { $diferencia = limpiar($_POST['diferencia']); } else { $diferencia =''; }
		if (limpiar(isset($_POST['comentarios']))) { $comentarios = limpiar($_POST['comentarios']); } else { $comentarios =''; }
...
{
	self::SetNames();
	if(empty($_POST["codarqueo"]) or empty($_POST["montoinicial"]) or empty($_POST["dineroefectivo"]))
	{
		echo "1";
		exit;
	}
...
############################ FUNCION REGISTRAR VENTAS ##############################
	public function NuevoPedido()
	{
		...
		$stmt->bindParam(24, $statuspago);
		$stmt->bindParam(25, $fechaventa);
		$stmt->bindParam(26, $delivery);
		$stmt->bindParam(27, $repartidor);
		$stmt->bindParam(28, $entregado);
		$stmt->bindParam(29, $observaciones);
		$stmt->bindParam(30, $codigo);
		$stmt->bindParam(31, $bandera);
...
############################ FUNCION CERRAR VENTAS ##############################
	public function CerrarMesa()
	{
		...
		$stmt->bindParam(12, $statusventa);
		$stmt->bindParam(13, $statuspago);
		$stmt->bindParam(14, $observaciones);
		$stmt->bindParam(15, $bandera);
		$stmt->bindParam(16, $codpedido);
...
public function DetallesPedidoMesa()
	...
	GROUP_CONCAT(cantventa, ' | ', substr(producto, 1,18) , ' | ', valorneto SEPARATOR '<br>') AS detalles, SUM(valorneto) AS suma 
	FROM ventas INNER JOIN detallepedidos ON detallepedidos.codpedido = ventas.codpedido 
	LEFT JOIN clientes ON ventas.codcliente = clientes.codcliente 
	LEFT JOIN mesas ON mesas.codmesa = ventas.codmesa 
	LEFT JOIN salas ON mesas.codsala = salas.codsala 
